...
if ( $type == 'nivo' ) {
    ...
    $el_start = '<li>';
    $el_end = '</li>';
    $slides_wrap_start = '<div class="shadows"><ul class="slides">';
...
while ( $my_query->have_posts() ) {
    ...
    // if ( $thumbnail == '' ) $thumbnail = __("No Featured image set.", "vh");

    // Link logic
    if ( $link != 'link_no' ) {
        if ( $link == 'link_post' ) {
            $link_image_start = '<a class="link_image" href="'.get_permalink($post_id).'" title="'.sprintf( esc_attr__( 'Permalink to %s', 'vh' ), the_title_attribute( 'echo=0' ) ).'">';
        }
...
else { $teasers = __("Nothing found." , "vh"); }
